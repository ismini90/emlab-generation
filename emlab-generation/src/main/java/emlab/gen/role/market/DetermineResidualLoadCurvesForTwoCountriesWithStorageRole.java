...
public class DetermineResidualLoadCurvesForTwoCountriesWithStorageRole extends AbstractRole<DecarbonizationModel>
    ...
    public void act(DecarbonizationModel model) {
        ...
        List<PowerGeneratingTechnology> storageTechnologyList = Utils
                .asList(reps.powerGeneratingTechnologyRepository.findAllStoragePowerGeneratingTechnologies());
        // logger.warn("Storage Technology List:" +
        // storageTechnologyList.toString());
        List<PowerGeneratingTechnology> technologyList = Utils.asList(
                reps.powerGeneratingTechnologyRepository.findAllStorageAndIntermittentPowerGeneratingTechnologies());
        ...
        if (!storagePowerPlantList.contains(null)) {
            ...
            try {
                ...
                // zone
                for (Zone zone : zoneList) {
                    P[zoneList.indexOf(zone)] = cplex.numVarArray(HOURS, zoneList.indexOf(zone),
                ...
                // i
                for (Zone zone : zoneList) {
                    E[zoneList.indexOf(zone)] = cplex.numVarArray(HOURS, zoneList.indexOf(zone),
                            zoneList.indexOf(zone));
                    for (int hour = 0; hour < HOURS; hour++) {
                        E[zoneList.indexOf(zone)][hour] = cplex.numVar(
        ...
        if (zoneList.size() > 1) {
            ...
            for (int row = 0; row < m.rows(); row++) {
                ...
                if ((smallerResidual <= 0) && biggerResidual <= 0) {
                    ...
                    m.viewColumn(IPROD.get(zoneBiggerResidual)).set(row,
                            m.get(row, LOADINZONE.get(zoneBiggerResidual)));
                ...
                m.viewColumn(RLOADTOTAL).set(row, m.get(row, RLOADINZONE.get(zoneSmallerResidual))
