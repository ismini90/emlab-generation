...
public interface PowerPlantRepository extends GraphRepository<PowerPlant> {
    ...
    @Query(value = "result=g.v(plant).in('REGARDING_POWERPLANT').propertyFilter('type', FilterPipe.Filter.EQUAL, 14).propertyFilter('time', FilterPipe.Filter.EQUAL, tick).money.sum(); if(result==null){result=0}else{result=result}; return result", type = QueryType.Gremlin)
    double calculateTenderRevenueOfPowerPlant(@Param("plant") PowerPlant plant, @Param("tick") long tick);
